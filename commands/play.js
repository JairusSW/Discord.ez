    ...
    try {
      ...
      try {
        ...
        if (getPlaylistID(args[0])) {
          ...
          const query = await ytdl.getBasicInfo(
            `https://youtube.com/watch?v=${args[0].trim()}`
          );
          ...
          song = {
            ...
            shortTitle: shortenTitle(
              Util.escapeMarkdown(query["videoDetails"].title)
            ),
      ...
      const play = async (song) => {
        ...
        queue.stream = await ytdlOpus(song.url, {
          quality: "lowestaudio",
          filter: "audioonly",
          highWaterMark: 1 << 25,
        });
        ...
        const dispatcher = queue.connection.play(queue.stream, {
          type: "opus",
        });
        ...
        dispatcher.on("finish", () => {
          ...
          if (queue.shuffle === false && queue.loop === false)
            queue["location"]++;

          if (
            queue.location >= queue.songs.length &&
            queue.loop === false &&
            queue.shuffle === false
          ) {
          ...
          const nextPlaying = new MessageEmbed()
            ...
            .addField(
              `Quality `,
              `${[
                song.bitrate.toString().split("")[0],
                song.bitrate.toString().split("")[1],
              ].join("")}kbps`
            )
