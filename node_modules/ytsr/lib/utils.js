...
exports.checkArgs = (searchString, options = {}) => {
  ...
  const inputURL = URL.parse(searchString, true);
  if (searchString.startsWith(BASE_URL) && inputURL.pathname === '/results' && inputURL.query.sp) {
    // Watch out for requests with a set filter
    // in such a case searchString would be an url including `sp` & `search_query` querys
    if (!inputURL.query.search_query) throw new Error('filter links have to include a "search_string" query');
    obj.query = inputURL.query;
  } else {
